...
class GamePage extends Component {
  state = {
    ...
    spawn: [],
    loading: true,
    x: [...Array(101).keys()].reverse(),
    y: [...Array(101).keys()].reverse(),
    positions: [],
    playerPos: [0,0],
    players: [],
    tokensPos: [],
  ...
  componentDidMount = async () => {
    ...
    const promisesHA = [];
    const lastId = await this.checkTokensHashAvatars();

    for(let i = 0; i < lastId; i++){
      promisesHA.push(this.checkTokensPos(this.state.hashAvatars,i));
    }
    await Promise.all(promisesHA)
    setInterval(async () => {
  ...
  checkTokensHashAvatars = async () => {
    const colletible = await this.props.snowflakesInvasion.methods.collectibles(0).call();
        ...
        <VStack spacing={3}>
          <Box>
            ...
            <Text fontSize="md" style={{wordBreak: 'break-all'}}>
              <p>Snowflakes are exploring the world and atacking each new occupied territory they find!</p>
              <p>HashNation, a peaceful nation, can be seen on the horizon. Lets steal and destroy it!</p>
              <p>Prepare to atack HashNation!</p>
              <p>Select a Snowflake(s) you own and atack the HashNation!</p>
              <p>Each movement needs to wait at least 50 blocks until next movement.</p>
              <p>Move to a field occupied by a HashAvatar to atack it!</p>
              <p>The unit that loose a battle is returned to the owner, the winner occupies the field occupied by the looser.</p>
              <p>Snowflakes units can not atack each other or occcupy same field.</p>
          ...
          <Box style={{overflow: 'scroll',maxWidth: '1200px',maxHeight: '500px'}}>
            <Table border="1" size="sm" style={{backgroundImage: `url('https://ipfs.io/ipfs/QmawshbGMd2SsDNLEYEzWnVu8jCQDozXFQZvuYEwdCfT3H')`,  backgroundRepeat: 'no-repeat',  backgroundSize: 'cover' , backgroundPosition: 'center'}}>
              <Thead>
                <Tr>
                  <Th>
                    <p>Spawn area snowflakes</p>
